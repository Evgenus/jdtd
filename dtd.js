...
var CheckValue = Check.$extend({
    ...
    create: function(context, container) {
        var element = $("<span>")
            .text(this.value);
        return element
            .addClass(context.styles.check_value);
    },
...
var CheckType = Check.$extend({
    ...
    create: function(context, data) {
        ...
        return this.provider.create(context, data_value);
    },
...
var CheckRule = Check.$extend({
    ...
    create: function(context, data) {
        ...
        return element
            .addClass(context.styles.check_rule);
    },
...
var RuleCollectItem = Class.$extend({
    __init__: function (id, name, key, value) {
        this.id = id;
        this.name = name;
        this.key = key;
        this.value = value;
...
var RuleCollect = Rule.$extend({
    ...
    convert: function(context, collection, container, cls) {
        for (var num in collection) {
            ...
            var item = cls(item_data.name, item_data.key_name, key, value);
...
var BaseVariant = Class.$extend({
    __init__: function(id, name, variant) {
        this.id = id;
        this.name = name;
...
var StaticVariant = BaseVariant.$extend({
    create: function(context, data) {
        var option = $('<option>')
            ...
            .attr('value', this.id);
        option.text(this.name ? this.name : this.id);
        return option
...
var RuleVariant = Rule.$extend({
    __init__: function (context, rule_data) {
        ...
        this.all = {};
        this.name = rule_data.name;
        for(var num in rule_data.static) {
            var item = rule_data.static[num];
            var variant_data = context.get_checker(item.variant);
            var variant = StaticVariant(
                item.name, 
                item.item_name, 
                variant_data);
            this.static[item.name] = variant;
            this.all[item.name] = variant; 
        }
        for(var num in rule_data.dynamic) {
            var item = rule_data.dynamic[num];
            var variant_data = context.get_checker(item.variant);
            var variant = DynamicVariant(
                item.name, 
                item.item_name, 
                variant_data);
            this.dynamic[item.name] = variant;
            this.all[item.name] = variant; 
        }
    },
    create: function(context, data) {
        ...
        if(notEmpty(this.dynamic)) {
            ...
            if(item_data && selected in this.dynamic) {
                holder.append(
                    this.dynamic[selected].variant.create(context, item_data)
    ...
    match_declaration: function(name, value) {
        if(isArray(value)) {
            ...
            for(var item in value) {
                item = value[item];
                checks.push([item, this.factory(item)]);
            }
        ...
        if(isObject(value)) {
            if(!any_of_values(value, isObject)) {
                ...
                iteritems(value, function(k, v) {
                    checks.push([k, self.factory(k), self.factory(v)]);
                });
